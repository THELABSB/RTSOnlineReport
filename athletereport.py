...
def app():
    ...
    ,orientation='h',marker_color='rgb(178,34,34)'),
                        plotly.graph_objects.Bar(
        name = 'Right',
        y = x,
        x = [client['QuadRIso'].iloc[0],client['HamRIso'].iloc[0]]
    ,orientation='h',marker_color='rgb(105,105,105)',marker_line_color='rgb(128,128,128)')
    ])
    plot.update_layout(autosize=False, height=300,width=550, title_text='Isometric Strength (N)')
    #endregion
    
    #region Distance Hops
    mecs = ['SL-Hop', 'Triple-Hop', 'Crossover-Hop']
    
    mechplot = plotly.graph_objects.Figure(data=[plotly.graph_objects.Bar(
        name = 'Left',
        x = mecs,
        y = [client['XHopLAvg'].iloc[0], client['TriHopLAvg'].iloc[0], client['SLHopLAvg'].iloc[0]]
    ,marker_color='rgb(178,34,34)'),
                        plotly.graph_objects.Bar(
        name = 'Right',
        x = mecs,
        y = [client['XHopRAvg'].iloc[0], client['TriHopRAvg'].iloc[0], client['SLHopRAvg'].iloc[0]]
    ,marker_color='rgb(105,105,105)',marker_line_color='rgb(128,128,128)')
    ])
    mechplot.update_layout(autosize=False, height=400,width=550, title_text='Hop Distance (in)')
    #endregion
    ...
    st.write('__Presented by Dr. Zach Finer, DPT__')
    st.header(name)
    ...
    if result == 'Fail':
        st.markdown('Based on the results for ' + client['Name'].to_string(index= False) + ' did not satisfy the requirements to safely return to sport.')
    elif result == 'Pass':
        st.markdown('Based on the results for ' + client['Name'].to_string(index= False) + ' satisfy the requirements to safely return to sport.')
    st.title(' ')    
    st.markdown('___')
    
    col1, col2, col3 =st.columns([0.8,1,2])

    col1.subheader('Primary Activity:  ' + client['Primary_Activity'].to_string(index= False))
    ...
    col1.markdown('Ankle Mobility ([cm]/[deg]): ')
    col1.markdown('Hip Internal Rotation: ')
    col1.markdown('Hip External Rotation: ')
    col2.title(' ')
    ...
    if quadisoasym <10:
        col1.markdown('**Quadiceps (Knee Extension) Isometric**:  Pass')
    elif quadisoasym >=10:
        col1.markdown('**Quadiceps (Knee Extension) Isometric**:  Fail')
    if hamisoasym <10:
        col1.markdown('**Hamstring (Knee Flexion) Isometric**:  Pass')
    elif hamisoasym >=10:
        col1.markdown('**Hamstring (Knee Flexion) Isometric**:  Fail')
    if slhopasym <10:
        col2.markdown('**Single Leg Hop**:  Pass')
    elif slhopasym >=10:
        col2.markdown('**Single Leg Hop**:  Fail')
    if trihopasym <10:
        col2.markdown('**Triple Hop**:  Pass')
    elif trihopasym >=10:
        col2.markdown('**Triple Hop**:  Fail')
    if xhopasym <10:
        col2.markdown('**Cross Hop**:  Pass')
    elif xhopasym >=10:
        col2.markdown('**Cross Hop**:  Fail')
    if timehopasym <10:
        col2.markdown('**Timed Hop**:  Pass')
    elif timehopasym >=10:
        col2.markdown('**Timed Hop**:  Fail')
    
    if pd.isna(client['Notes'].iloc[0]) is False:
        st.write(client['Notes'])
    
    st.write('___')
    st.subheader('Finer Protocol -  Results')
    ...
    select=col3.radio('Select',['Timed Hop','Single Leg Hop','Triple Hop','Cross Hop'])
    
    if select == 'Timed Hop':
        righthop= client['TimeHopRAvg'].iloc[0]
        lefthop= client['TimeHopLAvg'].iloc[0] 
    elif select== 'Single Leg Hop':
        righthop=client['SLHopRAvg'].iloc[0]
        lefthop= client['SLHopLAvg'].iloc[0]
    elif select=='Triple Hop':
        righthop= client['TriHopRAvg'].iloc[0]
        lefthop= client['TriHopLAvg'].iloc[0]
    elif select== 'Cross Hop':
        righthop=client['XHopRAvg'].iloc[0]
        lefthop= client['XHopLAvg'].iloc[0]
    
    
    labels = ['Right','Left']
    values = [righthop, lefthop]

    pie = plotly.graph_objects.Figure(data=[plotly.graph_objects.Pie(labels=labels, values=values, textinfo='label+percent',
